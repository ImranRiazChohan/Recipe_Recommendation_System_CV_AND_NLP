...
def get_recommendations(N, scores):
    ...
    top = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:N]
    # create dataframe to load in recommendations 
    recommendation = pd.DataFrame(columns = ['recipe', 'Ingredients', 'Cuisine', 'Diet','Course','score'])
    count = 0
    for i in top:
        recommendation.at[count, 'recipe'] = title_parser(df_recipes['RecipeName'][i])
        recommendation.at[count, 'ingredients'] = ingredient_parser_final(df_recipes['Ingredients'][i])
        recommendation.at[count, 'Cuisine'] = df_recipes['Cuisine'][i]
        recommendation.at[count, 'Course'] = df_recipes['Course'][i]
        recommendation.at[count, 'Diet'] = df_recipes['Diet'][i]
        recommendation.at[count, 'score'] = "{:.3f}".format(float(scores[i]))
        count += 1
    return recommendation
...
def RecSys(ingredients, N=5):
    ...
    # load in tdidf model and encodings 
    with open(config.INDIAN_TFIDF_ENCODING_PATH, 'rb') as f:
        tfidf_encodings = pickle.load(f)

    with open(config.INDIAN_TFIDF_MODEL_PATH, "rb") as f:
        tfidf = pickle.load(f)
